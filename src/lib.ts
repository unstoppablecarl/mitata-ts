...
export function do_not_optimize(v: any) { $._ = v; }
...
export async function generator(
  ...
  let $fn: unknown = value;

  if (typeof value === 'object' && value !== null) {
    if (!value.heap && value.heap != null) opts.heap = false;
    opts.concurrency ??= value.concurrency ?? opts.args?.concurrency;
    if (!value.counters && value.counters != null) opts.$counters = false;

    if (n.done || kind($fn) !== 'fn') {
      $fn = value.bench ?? value.manual;

      if (kind($fn, true) !== 'fn') throw new TypeError('expected benchmarkable yield from generator');

      opts.params ??= {};
      const params = ($fn as () => any).length;

      opts.manual = !value.manual ? false : value.budget !== 'manual' ? 'real' : 'manual';

      for (let o = 0; o < params; o++) {
        opts.params[o] = value[o];
        if (kind(value[o]) !== 'fn') throw new TypeError('expected function for benchmark parameter');
      }
    }
  }

  const stats = await fn($fn as any, opts);
...
export async function fn(fn: (...args: any[]) => any, opts: k_options = {}): Promise<Stats> {
  defaults(opts);
  let async = false;
  let batch = false;
  ...
  const loop = new AsyncFunction('$fn', '$gc', '$now', '$heap', '$params', '$counters', `
    ...
    ${!(opts.gc && opts.inner_gc && !(opts.gc as any).fallback) ? '' : 'const gc = { total: 0, min: Infinity, max: -Infinity };'}
    ...
    for (; _ < ${opts.max_samples}; _++) {
      ...
      ${!(opts.gc && opts.inner_gc && !(opts.gc as any).fallback) ? '' : `
        igc: {
          const t0 = $now();
          $gc(); const t1 = $now() - t0;
    ...
    return {
      ...
      ${!(opts.gc && opts.inner_gc && !(opts.gc as any).fallback) ? '' : 'gc: { ...gc, avg: gc.total / _ },'}
...
export async function iter(iter: (state: k_iter & k_statefree) => any, opts: k_options = {}): Promise<Stats> {
  const _ = {} as any;
  defaults(opts);
  let samples: number[] = new Array(2 ** 20);
  const _i = { next() { return (_ as any).next() } };
