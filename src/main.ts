...
import { kind, measure, print as grint } from './lib.js';

let FLAGS = 0;
let $counters = null;
let COLLECTIONS: CollectionItem[] = [{ id: 0, name: null, types: [], trials: [] }];
...
export class B {
  f: (() => any) | ((state: State) => Gen) | null = null
  _args = {};
  _name = '';
  _group = 0;
  _gc: GCType = 'once';
  flags = FLAGS;
  _highlight: Color | false = false;

  constructor(name: string, f: () => any);
  constructor(name: string, gen: (state: State) => Gen);
  constructor(name: string, f: (() => any) | ((state: State) => Gen)) {
    this.f = f;
    this.name(name);
    if (!kind(f)) throw new TypeError('expected iterator, generator or one-shot function');
  }

  name(name: string, color: Color | false = false) {
  ...
  highlight(color: Color | false = false) {
  ...
  // clear args[name]
  args(name: string, values: null): this
  // set args[name]
  args(name: string, values: any[]): this
  // set args
  args(map: Record<string, any[] | null>): this
  args(name: any, args?: any) {
    if (name === null) return (delete (this._args as any).x, this);
    if (Array.isArray(name)) return ((this._args as any).x = name, this);
  ...
  *_names() {
  ...
  async run(thrw = false): Promise<Trial> {
    const args = Object.keys(this._args);
    const kind: ArgsType = 0 === args.length ? 'static' : (1 === args.length ? 'args' : 'multi-args');
...
export function boxplot(f: () => any) { return _c(f, 'x'); }
export function barplot(f: () => any) { return _c(f, 'b'); }
export function summary(f: () => any) { return _c(f, 's'); }
...
function runtime() {
  ...
  if (globalThis.inIon && (globalThis.performance as any)?.mozMemory) return 'spidermonkey';
...
const formats = {
  ...
  async json(ctx: Context, opts, benchmarks, layout) {
      ...
      for (const trial of collection.trials) {
        if (opts.filter.test(trial._name)) {
          ...
          for (const r of bench.runs) {
            ...
            if (r.error) {
              if (!opts.colors) print(`${_h($.str(r.name, k_legend).padEnd(k_legend))} error: ${(r.error as any).message ?? r.error}`);
              else print(`${_h($.str(r.name, k_legend).padEnd(k_legend))} ${$.red + 'error:' + $.reset} ${(r.error as any).message ?? r.error}`);
            }
...
export const $ = {
  ...
  lineplot: {
    ...
    ascii(map, {
      colors = true as boolean | ColorBounds | Record<string, Color>,
      xmin = 0, xmax = 1,
      ymin = 0, ymax = 1,
      ...
      const plabels = {
        0: !(colors as ColorBounds)?.ymax ? (labels.ymax || '') : ((colors as ColorBounds).ymax + (labels.ymax || '') + $.reset),
        [lines.length - 1]: !(colors as ColorBounds)?.ymin ? (labels.ymin || '') : ((colors as ColorBounds).ymin + (labels.ymin || '') + $.reset),
      };
      ...
      if (labels.xmin || labels.xmax) {
        ...
        canvas.push(
          ' '.repeat(key) + ' '
          + (!(colors as ColorBounds)?.xmin ? xmin : (colors as ColorBounds).xmin + xmin + $.reset)
          + (!(colors as ColorBounds)?.xmax ? xmax.padStart(gap) : (colors as ColorBounds).xmax + xmax.padStart(gap) + $.reset)
        );
      }
  ...
  boxplot: {
    ...
    ascii(map: Record<string, Stats>, key = 8, size = 14, { fmt = $.time, colors = true, symbols = $.boxplot.symbols } = {}) {
      let tmin = Infinity;
      let tmax = -Infinity;
      const keys = Object.keys(map);
